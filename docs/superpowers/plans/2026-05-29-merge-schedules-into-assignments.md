# Merge Schedules into Assignments Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Consolidate the Schedules and Assignments views into a single "Assignments" page visible to all users, removing the Schedules nav link.

**Architecture:** `AssignmentsView.vue` absorbs the full content of `SchedulesView.vue` (schedule create form + list), placing it above the existing admin-only assignment cards. Shared data (tenantUsers, templates) is fetched once. NavBar removes the Schedules link and drops the admin guard on Assignments. The `/schedules` route becomes a redirect to `/assignments` for backward-compatibility.
...
**Modify:**
- `frontend/src/views/AssignmentsView.vue` — merge schedule sections in, import auth store for role-based visibility
- `frontend/src/components/NavBar.vue` — remove Schedules link; remove admin guard on Assignments link
- `frontend/src/router/index.js` — add `/schedules` → `/assignments` redirect

**No backend changes.**

---

### Task 1: Update NavBar.vue

**Files:**
- Modify: `frontend/src/components/NavBar.vue`

- [ ] **Step 1: Remove the Schedules link and drop the admin guard on Assignments**

Open `frontend/src/components/NavBar.vue`. Replace the nav-links block:
...
git commit -m "feat: remove Schedules nav link and make Assignments visible to all users"
```

---

### Task 2: Add /schedules redirect in router

**Files:**
- Modify: `frontend/src/router/index.js`

- [ ] **Step 1: Add the redirect entry and remove the SchedulesView import**

Open `frontend/src/router/index.js`. Replace the full file with:
...
git commit -m "feat: redirect /schedules to /assignments"
```

---

### Task 3: Merge schedule content into AssignmentsView.vue

**Files:**
- Modify: `frontend/src/views/AssignmentsView.vue`

- [ ] **Step 1: Replace the entire file with the merged content**

Replace `frontend/src/views/AssignmentsView.vue` with:
...
- [ ] **Step 2: Start the dev server and verify as a regular user**

```bash
cd frontend && npm run dev
```

Navigate to `http://localhost:5173` as a regular user. Confirm:
- NavBar shows: Dashboard, Reports, Files, Templates, Assignments (no Schedules link)
- `/assignments` loads with "Create Schedule" and "Your Schedules" cards
- "Assign Report Task" and "All Assignments" cards are NOT visible
- Navigating to `http://localhost:5173/schedules` redirects to `/assignments`

- [ ] **Step 3: Verify as admin**

Log in as an admin. Confirm:
- NavBar shows Assignments (no Schedules)
- `/assignments` shows all four cards: Create Schedule, Your Schedules, Assign Report Task, All Assignments
- Creating a schedule still works (form submits, table refreshes)
- Creating an assignment still works (form submits, table refreshes)

- [ ] **Step 4: Commit**

```bash
git add frontend/src/views/AssignmentsView.vue
git commit -m "feat: merge Schedules into Assignments view"
```
...
Add the following block at the top of `docs/superpowers/plans/CHANGELOG.md` (after the `# DocPlatform Changelog` heading):
...
- `frontend/src/views/AssignmentsView.vue` — merged schedule sections; role-gated admin sections
...
git commit -m "docs: update CHANGELOG for schedules-into-assignments merge"
```
